...
void magic(char *veccar, int *index, int *counter, char car, int size, int *index2, char *auxalphabet)
{
    int i=0;
    if (*counter == 26)
        return;
    ...
    if (veccar[*index + 1] != car)
        magic(veccar, index, counter, unalterable_pointer[ *index2 = *index2 + 1], size, index2,auxalphabet);
    if (veccar[*index + 1] == car )
    {
        for(i=0;i<26;i++) {
            if (auxalphabet[i] == car) {
                *counter = *counter-1;
                break;
                }
            };
        auxalphabet[*counter+1] = car;
        *counter = *counter + 1;
        *index2 = 0;
        *index = *index + 1;
...
void main(int argc, char *argv[])
{
    ...
    int index = 0;
    int index2 = 0;
    char *auxalphabet =calloc(27,sizeof(char));
    if (argc > 2 || argc < 2)
        return;
